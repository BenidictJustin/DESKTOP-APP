...
export async function preparePrintHtmlPayload(element, title, options = {}) {
  ...
  let pagesHtml = ''
  let docW = options.paperW || 816
  let docH = options.paperH || 1248
  let docxSections = []
  let docxCustomStyles = ''
  ...
  if (isSimpleTest) {
    ...
    docxSections = Array.from(cloned.querySelectorAll('section.docx'))

    if (docxSections.length > 0) {
      // Capture any inline or embedded <style> tags generated by docx-preview
      const styleEls = Array.from(cloned.querySelectorAll('style')).concat(
        element ? Array.from(element.querySelectorAll('style')) : []
      )
      const seenStyles = new Set()
      styleEls.forEach((st) => {
        if (st && st.textContent && !seenStyles.has(st.textContent)) {
          seenStyles.add(st.textContent)
          docxCustomStyles += st.textContent + '\n'
        }
      })

      if (docxSections[0]) {
        const s0 = docxSections[0]
        const pw = parseFloat(s0.style.width)
        const ph = parseFloat(s0.style.minHeight || s0.style.height)
        if (pw > 0) docW = Math.round(pw)
        if (ph > 0) docH = Math.round(ph)
      }

      docxSections.forEach((section) => {
        const sClone = section.cloneNode(true)
        const srcCanvases = section.querySelectorAll('canvas')
        const cloneCanvases = sClone.querySelectorAll('canvas')
        srcCanvases.forEach((srcCanvas, cIdx) => {
          try {
            ...
            if (cloneCanvases[cIdx]) {
              cloneCanvases[cIdx].replaceWith(img)
            }
          } catch (e) {
            console.warn('Canvas toDataURL failed in print clone:', e)
          }
        })
  ...
  const getCssPageSize = () => {
    if (docxSections && docxSections.length > 0) {
      const wIn = (docW / 96).toFixed(2)
      const hIn = (docH / 96).toFixed(2)
      if (options.landscape) {
        return `${hIn}in ${wIn}in`
      }
    ...
    <html>
      <head>
        ...
        <style>
          ...
          /* Explicitly ensure DOCX headers, footers, logos, tables, and images are NEVER hidden */
          header.docx, .docx header, section.docx header, .docx-header {
            display: block !important;
            visibility: visible !important;
            opacity: 1 !important;
          }
          footer.docx, .docx footer, section.docx footer, .docx-footer {
            display: block !important;
            visibility: visible !important;
            opacity: 1 !important;
          }
          section.docx img, .docx-render-target img {
            display: inline-block !important;
            visibility: visible !important;
            max-width: 100% !important;
          }
          section.docx table, .docx-render-target table {
            border-collapse: collapse !important;
          }

          .no-print, button, input[type="button"], select, aside, .doc-viewer-modal > header, .doc-viewer-modal header.bg-white, header:not(.docx):not(.docx-header), .doc-viewer-modal {
            display: none !important;
          }
...
/** Export the rendered element as a DOCX document via Word COM automation preserving exact layout. */
export async function exportElementToDOCX(element, title, options = {}) {
  if (!element) return

  try {
    if (!window.electron?.ipcRenderer && !window.api?.exportHtmlToDocx) {
      throw new Error('Electron ipcRenderer is not available in this context.')
    }
    // Reuses the EXACT same DOM scraping, CSS, and base64 asset serialization as working Export PDF
    const payload = await preparePrintHtmlPayload(element, title, options)
    if (!payload) return

    const { htmlContent, formattedTitle, ipcPageSize } = payload
    const sendData = {
      html: htmlContent,
      title: formattedTitle,
      options: {
        paperKey: options.paperKey || (ipcPageSize === 'Folio' ? 'Folio' : ipcPageSize) || 'Folio',
        orientation: options.landscape ? 'landscape' : 'portrait',
        marginKey: options.marginKey || 'Normal',
        paperW: options.paperW || 816,
        paperH: options.paperH || 1248
      }
    }

    const result = window.api?.exportHtmlToDocx
      ? await window.api.exportHtmlToDocx(sendData)
      : await window.electron.ipcRenderer.invoke('export-html-to-docx', sendData)

    return result
  } catch (error) {
    console.error('export-html-to-docx IPC failed:', error)
    throw error
  }
}
...
/**
 * Convert and export a DOCX document directly to PDF.
 * Tries Native MS Word COM Engine first (100% pixel-perfect), then falls back to in-app high-DPI vector PDF generator.
 */
export async function exportDocxToPDF(docxUrl, title, docxContainerElement) {
  const cleanTitle = (title || 'Document').replace(/\.docx$/i, '').replace(/[^a-zA-Z0-9_-]+/g, '_')

  // 1. Try Native Microsoft Word COM Conversion on Windows
  try {
    if (window.electron?.ipcRenderer) {
      const buffer = await getDocxArrayBuffer(docxUrl)
      if (buffer) {
        const res = await window.electron.ipcRenderer.invoke('convert-docx-to-pdf-native', {
          buffer,
          fileName: cleanTitle
        })
        if (res?.success) {
          return { success: true, filePath: res.filePath, engine: 'native_word' }
        }
        if (res?.cancelled) {
    ...
    console.warn('Native Word COM conversion attempt failed, falling back to in-app renderer:', err)
  }

  // 2. High-Fidelity In-App Fallback via docx-preview container
  if (docxContainerElement) {
    return await exportElementToPDF(docxContainerElement, cleanTitle, { isDocument: true })
  }

  // 3. Temporary Hidden Render Container Fallback
  const hiddenContainer = document.createElement('div')
  hiddenContainer.style.position = 'fixed'
  hiddenContainer.style.top = '-9999px'
  hiddenContainer.style.left = '-9999px'
  hiddenContainer.style.width = '816px'
  ...
  try {
    ...
    const buffer = await getDocxArrayBuffer(docxUrl)
    if (!buffer) throw new Error('Unable to read document data')
    await renderAsync(buffer, hiddenContainer, null, {
      breakPages: true,
      renderHeaders: true,
      renderFooters: true,
    ...
    const sections = hiddenContainer.querySelectorAll('section.docx')
    sections.forEach((s) => {
    ...
    return await exportElementToPDF(hiddenContainer, cleanTitle, { isDocument: true })
  } finally {
