...
export default function StatusBar({
  wordCount, charCount, paperKey, orientation, marginKey, zoom, setZoom,
  loading, workspaceIsReadOnly, onSaveDraft, onSubmit,
  ...
  return (
    <div className="bg-navy-blue text-gray-300 flex items-center justify-between px-4 py-1 text-[10px] shrink-0 select-none">
      <div className="flex items-center gap-4">
        ...
        <span>Paper: <strong className="text-white">{paperKey}{orientation === 'landscape' ? ' (L)' : ''}</strong></span>
        <span>Margins: <strong className="text-white">{marginKey}</strong></span>
      </div>
      <div className="flex items-center gap-3">
        {!workspaceIsReadOnly && (
          <>
            <button onClick={onSaveDraft} disabled={loading}
              ...
              <Save className="w-3 h-3" /><span>Save Draft</span>
            </button>
        ...
        <div className="flex items-center gap-1">
          <button onClick={() => setZoom(z => Math.max(50, z - 10))} className="hover:text-white cursor-pointer transition">
            <ZoomOut className="w-3 h-3" />
          </button>
          <span className="w-8 text-center"><strong className="text-white">{zoom}%</strong></span>
          <button onClick={() => setZoom(z => Math.min(200, z + 10))} className="hover:text-white cursor-pointer transition">
