...
export default function RibbonHome({ editor, lineSpacing, setLineSpacing, onOpenFindReplace }) {
  ...
  const currentFont =
    editor?.getAttributes('textStyle').fontFamily?.split(',')[0]?.replace(/"/g, '') || 'Calibri'
  const currentSize =
    editor?.getAttributes('textStyle').fontSize?.replace('px', '').replace('pt', '') || '11'
    ...
    <div className="flex items-end gap-0 overflow-visible flex-nowrap">
        ...
        <div className="relative" ref={fontRef}>
          ...
          >
            <span
              ...
              style={{
                fontFamily: editor?.getAttributes('textStyle').fontFamily || 'Calibri, sans-serif'
              }}
            >
              {currentFont}
            </span>
                ...
                <button
                  ...
                  onClick={() => {
                    editor
                      ?.chain()
                      .focus()
                      .setMark('textStyle', { fontSize: `${s}pt` })
                      .run()
                    setShowSizeDD(false)
                  }}
      ...
      <RGroup label="Format">
        ...
        <div className="relative" ref={highlightRef}>
          ...
          <DropdownWrapper
            open={showHighlightDD}
            onClose={() => setShowHighlightDD(false)}
      ...
      <RGroup label="Styles">
        <div className="relative" ref={headingRef}>
          ...
          <DropdownWrapper
            open={showHeadingDD}
            onClose={() => setShowHeadingDD(false)}
      ...
      <RGroup label="Spacing">
        <div className="relative" ref={spacingRef}>
          ...
          <DropdownWrapper
            open={showSpacingDD}
            onClose={() => setShowSpacingDD(false)}
            triggerRef={spacingRef}
            width={150}
          >
            <div className="py-1 w-36">
              {LINE_SPACINGS.map((ls) => (
                <button
                  key={ls.value}
                  onClick={() => {
                    setLineSpacing(ls.value)
                    setShowSpacingDD(false)
                  }}
