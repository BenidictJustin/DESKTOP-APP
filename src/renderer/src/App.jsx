...
function App() {
  ...
  return (
    <>
      ...
      {loading ? (
        <div className="min-h-screen w-screen flex flex-col items-center justify-center font-poppins bg-[#030728]" />
      ) : !user ? (
        <Login onLoginSuccess={setUser} />
      ) : user.role === 'admin' ? (
        <AdminDashboard user={user} onLogout={handleLogout} />
      ) : user.role === 'office_coordinator' ? (
        <OfficeCoordinatorDashboard user={user} onLogout={handleLogout} />
      ) : (
